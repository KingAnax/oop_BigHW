...
static int gmw_inner_draw_block_for_move(const CONSOLE_GRAPHICS_INFO* const pCGI, const int BX, const int BY, const int bdi_value_catchy, const BLOCK_DISPLAY_INFO* const bdi)
{
	...
	for (int i = 0; (bdi + i)->value != BDI_VALUE_END; i++)
	{
		if (bdi_value_catchy == (bdi + i)->value) {
			...
			if (pCGI->CBI.block_border == false) {  //不需要边框
...
int gmw_status_line(const CONSOLE_GRAPHICS_INFO* const pCGI, const int type, const char* msg, const char* catchy_msg)
{
	...
	if (type == LOWER_STATUS_LINE && pCGI->lower_status_line) { //下状态栏
	...
	else if (type == TOP_STATUS_LINE && pCGI->top_status_line) { //上状态栏
...
/***************************************************************************
  ...
  输入参数：const CONSOLE_GRAPHICS_INFO *const pCGI	：整体结构指针
		   ...
		   const int col_no						：列号（从0开始，人为保证正确性，程序不检查）
		   const int bdi_value_catchy						：需要显示的值
		   const int blank_bdi_value_catchy				：移动过程中用于动画效果显示时用于表示空白的值（一般为0，此处做为参数代入，是考虑到可能出现的特殊情况）
